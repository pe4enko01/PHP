...
<div class="first-block">
    ...
    <div class="container">
        ...
        <div class="add-task">
            <form id="registerForm" action="index_name.php" method="POST" enctype="multipart/form-data">
                <p class="form-input">
        ...
        <div class="task-container">
            ...
            $query = mysqli_query($con, "SELECT * FROM taask ORDER BY username DESC   LIMIT $start, $num");
            ...
            if ($page != 1) $pervpage = '<a href=index_name.php?page=1>Первая</a> | <a href=index_name.php?page=' . ($page - 1) . '>Предыдущая</a> | ';
            // Проверяем нужны ли стрелки вперед
            if ($page != $total) $nextpage = ' | <a href=index_name.php?page=' . ($page + 1) . '>Следующая</a> | <a href=index_name.php?page=' . $total . '>Последняя</a>';

            // Находим две ближайшие станицы с обоих краев, если они есть
            if ($page - 5 > 0) $page5left = ' <a href=index_name.php?page=' . ($page - 5) . '>' . ($page - 5) . '</a> | ';
            if ($page - 4 > 0) $page4left = ' <a href=index_name.php?page=' . ($page - 4) . '>' . ($page - 4) . '</a> | ';
            if ($page - 3 > 0) $page3left = ' <a href=index_name.php?page=' . ($page - 3) . '>' . ($page - 3) . '</a> | ';
            if ($page - 2 > 0) $page2left = ' <a href=index_name.php?page=' . ($page - 2) . '>' . ($page - 2) . '</a> | ';
            if ($page - 1 > 0) $page1left = '<a href=index_name.php?page=' . ($page - 1) . '>' . ($page - 1) . '</a> | ';

            if ($page + 5 <= $total) $page5right = ' | <a href=index_name.php?page=' . ($page + 5) . '>' . ($page + 5) . '</a>';
            if ($page + 4 <= $total) $page4right = ' | <a href=index_name.php?page=' . ($page + 4) . '>' . ($page + 4) . '</a>';
            if ($page + 3 <= $total) $page3right = ' | <a href=index_name.php?page=' . ($page + 3) . '>' . ($page + 3) . '</a>';
            if ($page + 2 <= $total) $page2right = ' | <a href=index_name.php?page=' . ($page + 2) . '>' . ($page + 2) . '</a>';
            if ($page + 1 <= $total) $page1right = ' | <a href=index_name.php?page=' . ($page + 1) . '>' . ($page + 1) . '</a>';
